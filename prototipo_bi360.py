...
for month in monthly_data:
    ...
    month["auto_pct"] = (
        month["automatic"] / month["total"] * 100 if month["total"] > 0 else 0
    )

    unique_cols.update(
        ["posted", "not_posted", "total", "automatic", "manual", "auto_pct"]
    )
...
automation_stacked_data = {
    "Tipo de Documento": [
        {
            "Category": "NFS-e",
            "Sem Intervenção Manual": 850,
            "Com Intervenção Manual": 150,
        },
        {
            "Category": "CT-e",
            "Sem Intervenção Manual": 600,
            "Com Intervenção Manual": 400,
        },
        {
            "Category": "NF-e",
            "Sem Intervenção Manual": 700,
            "Com Intervenção Manual": 200,
        },
        {
            "Category": "Fatura",
            "Sem Intervenção Manual": 300,
            "Com Intervenção Manual": 350,
        },
    ],
    "Unidade de Negócio": [
        {
            "Category": "Unidade SP",
            "Sem Intervenção Manual": 1200,
            "Com Intervenção Manual": 300,
        },
        {
            "Category": "Unidade RJ",
            "Sem Intervenção Manual": 950,
            "Com Intervenção Manual": 250,
        },
        {
            "Category": "Unidade MG",
            "Sem Intervenção Manual": 400,
            "Com Intervenção Manual": 500,
        },
    ],
    "Nome do Fluxo": [
        {
            "Category": "Fluxo Padrão",
            "Sem Intervenção Manual": 2000,
            "Com Intervenção Manual": 200,
        },
        {
            "Category": "Fluxo Exceção",
            "Sem Intervenção Manual": 100,
            "Com Intervenção Manual": 600,
        },
        {
            "Category": "Fluxo Simplificado",
            "Sem Intervenção Manual": 450,
            "Com Intervenção Manual": 50,
        },
    ],
}
...
captura_stacked_data = {
    "Tipo de Documento": [
        {
            "Category": "Serviço",
            "Captura Automática": 8500,
            "Email": 2000,
            "Inserção Manual": 650,
        },
        {
            "Category": "Material",
            "Captura Automática": 6000,
            "Email": 1500,
            "Inserção Manual": 400,
        },
        {
            "Category": "Transporte",
            "Captura Automática": 4000,
            "Email": 800,
            "Inserção Manual": 250,
        },
        {
            "Category": "Concessionárias",
            "Captura Automática": 1200,
            "Email": 250,
            "Inserção Manual": 60,
        },
    ],
    "Unidade de Negócio": [
        {
            "Category": "Unidade SP",
            "Captura Automática": 10000,
            "Email": 2500,
            "Inserção Manual": 800,
        },
        {
            "Category": "Unidade RJ",
            "Captura Automática": 7500,
            "Email": 1800,
            "Inserção Manual": 450,
        },
        {
            "Category": "Unidade MG",
            "Captura Automática": 2200,
            "Email": 250,
            "Inserção Manual": 110,
        },
    ],
...
def get_detailed_task_data():
    ...
    for i in range(200):
        ...
        finalization_date_task = creation_date_task + timedelta(
            minutes=random.randint(5, 5 * 24 * 60)
        )
        ...
        data.append(
            {
                ...
                "Data de Criação da Tarefa": creation_date_task.strftime(
                    "%d/%m/%Y %H:%M"
                ),
                "Data de Finalização da Tarefa": finalization_date_task.strftime(
                    "%d/%m/%Y %H:%M"
                ),
        ...
        data.append(
            {
                ...
                "Status": random.choice(
                    ["Em Análise", "Corrigida", "Aguardando Fornecedor"]
                ),
                "Analista Responsável": random.choice(
...
open_notes_by_status_data = {
    "Unidade de Negócio": [
        {
            "Category": "Unidade SP",
            "Aprovação Pendente": 1500,
            "Análise Fiscal": 800,
            "Aguardando Pedido": 400,
        },
        {
            "Category": "Unidade RJ",
            "Aprovação Pendente": 1200,
            "Análise Fiscal": 650,
            "Aguardando Pedido": 350,
        },
        {
            "Category": "Unidade MG",
            "Aprovação Pendente": 800,
            "Análise Fiscal": 400,
            "Aguardando Pedido": 700,
        },
    ]
}
        ...
        data.append(
            {
                ...
                "Usuário Responsável": random.choice(
                    ["Daniel Souza", "Eduarda Lima", "Sistema"]
                ),
            }
        )
...
def create_bar_line_chart(
    ...
    for key, name, color in zip(bar_keys, bar_names, bar_colors):
        ...
        fig.add_trace(
            go.Bar(x=months, y=bar_values, name=name, marker_color=color),
            secondary_y=False,
        )
    ...
    fig.update_yaxes(
        title_text="Quantidade de Notas",
        secondary_y=False,
        color="#d1d5db",
        showgrid=False,
    )
...
def create_styled_list_table(title, data, primary_col, metric_cols, table_id):
    ...
    header = html.Div(
        ...
        children=[
            html.Button(
                col,
                id={"type": "sort-btn", "table_id": table_id, "col": col},
                n_clicks=0,
                style={
                    "background": "none",
                    "border": "none",
    ...
    rows = [
        html.Div(
            ...
            children=[
                html.P(
                    item[primary_col],
                    style={"flex": "2", "margin": "0", "fontSize": "0.9em"},
                ),
                html.P(
                    f"{item[metric_cols[0]]:,}".replace(",", "."),
                    style={
                        "flex": "1",
                        "textAlign": "right",
                        "margin": "0",
    ...
    return html.Div(
        [
            ...
            html.Div(
                rows,
                id={"type": "list-table-body", "table_id": table_id},
...
def create_captura_kpi_layout(totals):

    total_invoices = totals.get("total_invoices", 0)

    automatic_invoices = totals.get("automatic_invoices", 0)
        ...
        children=[
            html.Div(
                [
                    ...
                    html.P(
                        "notas foram escrituradas sem nenhuma ação manual",
                        style={"margin": "0", "color": "#9ca3af", "fontSize": "1em"},
                    ),
                ]
            ),
            html.Div(
                [
                    ...
                    html.P(
                        "das 12.356 notas escrituradas no período",
                        style={"margin": "0", "color": "#9ca3af"},
                    ),
                ]
            ),
        ...
        children=[
            html.Div(
                [
                    ...
                    html.P(
                        "Notas apresentaram divergências",
                        style={"margin": "0", "color": "#9ca3af", "fontSize": "1em"},
                    ),
                ]
            ),
                ...
                children=[
                    html.Div(
                        [
                            ...
                            html.P(
                                "Idade média", style={"color": "#9ca3af", "margin": "0"}
                            ),
                        ]
                    ),
                    html.Div(
        ...
        children=[
            html.Div(
                [
                    ...
                    html.P(
                        "Dias úteis - Leadtime médio geral",
                        style={"margin": "0", "color": "#9ca3af", "fontSize": "1em"},
                    ),
                ]
            ),
        ...
        return html.Div(
            [
                create_filter_box(
                    "Agrupamento",
                    options=[
                        {"label": k, "value": k} for k in automation_stacked_data.keys()
                    ],
        ...
        return html.Div(
            [
                create_filter_box(
                    "Agrupamento",
                    options=[
                        {"label": k, "value": k} for k in captura_stacked_data.keys()
                    ],
                    placeholder="Selecione...",
...
app.layout = html.Div(
    ...
    children=[
        ...
        dcc.Store(
            id="table-columns-store", data={"visible_columns": analytic_table_columns}
        ),
        dcc.Store(
            id="task-table-columns-store",
            data={"visible_columns": task_analytic_columns},
        ),
        dcc.Store(
            id="divergence-table-columns-store",
            data={"visible_columns": divergence_analytic_columns},
        ),
        dcc.Store(
            id="open-notes-table-columns-store",
            data={"visible_columns": open_notes_analytic_columns},
        ),
        dcc.Store(
            id="leadtime-table-columns-store",
            data={"visible_columns": leadtime_analytic_columns},
        ),
            ...
            children=[
                ...
                dbc.ModalBody(
                    dcc.Checklist(
                        ...
                        options=[
                            {"label": col, "value": col}
                            for col in analytic_table_columns
                        ],
                        value=analytic_table_columns,
                ...
                dbc.ModalFooter(
                    [
                        ...
                        dbc.Button("Fechar", id="close-config-button"),
                    ]
                ),
            ],
        ),
        dbc.Modal(
            ...
            children=[
                dbc.ModalHeader(
                    "Configurar Colunas da Tabela de Tarefas (Automatismo)"
                ),
                dbc.ModalBody(
                    dcc.Checklist(
                        id="task-column-checklist",
                        options=[
                            {"label": col, "value": col}
                            for col in task_analytic_columns
                        ],
                        value=task_analytic_columns,
                        labelStyle={"display": "block", "marginBottom": "5px"},
                    )
                ),
                dbc.ModalFooter(
                    [
                        ...
                        dbc.Button("Fechar", id="close-task-config-button"),
                    ]
                ),
            ],
        ),
        dbc.Modal(
            id="divergence-config-modal",
            size="lg",
            is_open=False,
            children=[
                ...
                dbc.ModalBody(
                    dcc.Checklist(
                        id="divergence-column-checklist",
                        options=[
                            {"label": col, "value": col}
                            for col in divergence_analytic_columns
                        ],
                        value=divergence_analytic_columns,
                        labelStyle={"display": "block", "marginBottom": "5px"},
                    )
                ),
                dbc.ModalFooter(
                    [
                        dbc.Button(
                            "Salvar",
                            id="save-divergence-config-button",
                            className="mr-1",
                            style={
                                "backgroundColor": "#8B5CF6",
                                "borderColor": "#8B5CF6",
                        ...
                        dbc.Button("Fechar", id="close-divergence-config-button"),
                    ]
                ),
            ],
        ),
        dbc.Modal(
            id="open-notes-config-modal",
            size="lg",
            is_open=False,
            children=[
                ...
                dbc.ModalBody(
                    dcc.Checklist(
                        id="open-notes-column-checklist",
                        options=[
                            {"label": col, "value": col}
                            for col in open_notes_analytic_columns
                        ],
                        value=open_notes_analytic_columns,
                        labelStyle={"display": "block", "marginBottom": "5px"},
                    )
                ),
                dbc.ModalFooter(
                    [
                        dbc.Button(
                            "Salvar",
                            id="save-open-notes-config-button",
                            className="mr-1",
                            style={
                                "backgroundColor": "#8B5CF6",
                                "borderColor": "#8B5CF6",
                        ...
                        dbc.Button("Fechar", id="close-open-notes-config-button"),
                    ]
                ),
            ],
        ),
        dbc.Modal(
            id="leadtime-config-modal",
            size="lg",
            is_open=False,
            children=[
                ...
                dbc.ModalBody(
                    dcc.Checklist(
                        id="leadtime-column-checklist",
                        options=[
                            {"label": col, "value": col}
                            for col in leadtime_analytic_columns
                        ],
                        value=leadtime_analytic_columns,
                        labelStyle={"display": "block", "marginBottom": "5px"},
                    )
                ),
                dbc.ModalFooter(
                    [
                        dbc.Button(
                            "Salvar",
                            id="save-leadtime-config-button",
                            className="mr-1",
                            style={
                                "backgroundColor": "#8B5CF6",
                                "borderColor": "#8B5CF6",
                        ...
                        dbc.Button("Fechar", id="close-leadtime-config-button"),
                    ]
                ),
            ],
        ),
        ...
        dcc.Tabs(
            ...
            children=[
                ...
                dcc.Tab(
                    label="Automatismo",
                    value="tab-automatismo",
                    style=tab_style,
                    selected_style=selected_tab_style,
                ),
                dcc.Tab(
                    label="Divergências",
                    value="tab-divergencias",
                    style=tab_style,
                    selected_style=selected_tab_style,
                ),
                dcc.Tab(
                    label="Notas em Aberto",
                    value="tab-notas-aberto",
                    style=tab_style,
                    selected_style=selected_tab_style,
                ),
                dcc.Tab(
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                html.H3(
                                    "Filtros",
                                    style={"textAlign": "center", "color": "#d1d5db"},
                                ),
                                create_filter_box(
                                    "Data de Criação",
                                    [
...
def create_stories_layout():
    ...
    story1 = html.Div(
        ...
        children=[
            dcc.Store(
                id={"type": "panel-state", "index": 1},
                data={"visible": False, "content": None},
            ),
            html.Div(
                ...
                children=[
                    html.Div(
                        ...
                        children=[
                            create_captura_kpi_layout(totals),
                            dcc.Graph(
                        ...
                        children=[
                            ...
                            dcc.Graph(
                                ...
                                figure=create_pie_chart(
                                    ...
                                    [11150, 7900, 5050, 1510],
                                    ["#00b894", "#636e72", "#fdcb6e", "#d63031"],
    ...
    story3 = html.Div(
        ...
        children=[
            dcc.Store(
                id={"type": "panel-state", "index": 3},
                data={"visible": False, "content": None},
            ),
            dcc.Store(
                id={"type": "sort-state", "table_id": "fornecedores"},
                data={"col": "Total de Notas", "order": "desc"},
            ),
            dcc.Store(
                id={"type": "sort-state", "table_id": "prefeituras"},
                data={"col": "Total de Notas", "order": "desc"},
            ),
            html.Div(
                style={
                    ...
                    html.Div(
                        ...
                        children=[
                            create_styled_list_table(
                                ...
                                fornecedores_data,
                                "Razão Social Fornecedor",
                                ["Total de Notas", "Percentual de Ingresso Manual"],
                                "fornecedores",
                            ),
                            create_styled_list_table(
                                "Top Prefeituras",
    ...
    manual_tasks_modal = dbc.Modal(
        [
            ...
            dbc.ModalBody(
                [
                    html.P(
                        "Selecione as tarefas a serem consideradas no cálculo de 'Com Intervenção Manual'."
                    ),
                    dcc.Checklist(
                        id="manual-task-checklist",
                        options=[
                            {"label": task, "value": task} for task in manual_task_names
                        ],
            ...
            dbc.ModalFooter(
                dbc.Button("Fechar", id="close-manual-task-modal", className="ml-auto")
            ),
        ],
        id="modal-manual-tasks",
        is_open=False,
    )

    story1 = html.Div(
        style={"backgroundColor": "#1f2937", "borderRadius": "8px", "padding": "20px"},
        children=[
            manual_tasks_modal,
            html.Div(
                style={"display": "flex", "alignItems": "stretch"},
                                ...
                                children=[
                                    dcc.Graph(
                                        ...
                                        figure=create_bar_line_chart(
                                            ...
                                            ["#2dd4bf", "#fb923c"],
                                            "#a78bfa",
                                            "Evolução Mensal do Automatismo",
                                        ),
                                        config={"displayModeBar": False},
                                        style={"height": "350px"},
                                    ),
                ...
                children=[
                    html.Div(
                        ...
                        children=[
                            ...
                            dcc.Graph(
                                figure=create_pie_chart(
                                    ["Tarefas Manuais", "Tarefas Automáticas"],
                                    [40, 60],
                                    ["#fb923c", "#2dd4bf"],
        ...
        children=[
            html.Div(
                ...
                children=[
                    create_styled_list_table(
                        "Top Tarefas Manuais",
                        get_top_manual_tasks_data(),
                        "Tarefa",
                        ["Ocorrências", "Tempo Médio (horas)"],
                        "top-tasks",
                    ),
                    create_styled_list_table(
                        "Top Analistas",
                        get_top_analysts_data(),
                        "Analista",
                        ["Tarefas Concluídas", "Tempo Médio (min)"],
                        "top-analysts",
                    ),
                ],
            )
                        ...
                        children=[
                            dcc.Graph(
                                figure=create_simple_bar_chart(
                                    pd.DataFrame(
                                        divergence_type_data["Tipo de Divergência"]
                                    ),
                                    x_key="Category",
                                    y_key="Ocorrências",
                                    color="#fb923c",
                            ...
                            dcc.Graph(
                                figure=create_pie_chart(
                                    ...
                                    "Distribuição Percentual de Divergências",
                                    hole=0.5,
                                ),
                                config={"displayModeBar": False},
                                style={"height": "400px"},
        ...
        children=[
            html.Div(
                ...
                children=[
                    html.Button(
                        html.I(className="fas fa-cog"),
                        id="open-divergence-config-button",
                        n_clicks=0,
                        style={
                            "border": "none",
                            "background": "none",
                ...
                children=[
                    html.Div(
                        ...
                        children=[
                            dcc.Graph(
                                figure=create_stacked_bar_chart(
                                    data=open_notes_by_status_data[
                                        "Unidade de Negócio"
                                    ],
                                    x_key="Category",
                                    y_keys=[
        ...
        children=[
            html.Div(
                ...
                children=[
                    html.Button(
                        html.I(className="fas fa-cog"),
                        id="open-open-notes-config-button",
                        n_clicks=0,
                        style={
                            "border": "none",
                            "background": "none",
...
def create_leadtime_layout():
                        ...
                        children=[
                            ...
                            dcc.Graph(
                                figure=create_multi_line_chart(
                                    ...
                                    [
                                        "Leadtime Médio (dias)",
                                        "Leadtime com Divergência (dias)",
                                        "Leadtime sem Divergência (dias)",
                                    ],
    ...
    story3 = html.Div(
        ...
        children=[
            ...
            html.P(
                "Esta seção pode incluir tabelas de leadtime por analista, por tipo de divergência, etc.",
        ...
        children=[
            html.Div(
                ...
                children=[
                    html.Button(
                        html.I(className="fas fa-cog"),
                        id="open-leadtime-config-button",
                        n_clicks=0,
                        style={
                            "border": "none",
                            "background": "none",
...
def toggle_kpi_side_panel(filter_clicks, info_clicks, data, button_id_dict):
    ...
    if not is_visible:
        ...
        new_children = (
            get_side_panel_content_for_index(button_id_dict["index"])
            if clicked_button_type == "filter"
            else get_side_panel_info_content()
        )
...
@app.callback(
    ...
    [
        Input("open-config-button", "n_clicks"),
...
@app.callback(
    ...
    [
        Input("open-task-config-button", "n_clicks"),
        Input("close-task-config-button", "n_clicks"),
...
@app.callback(
    ...
    [
        Input("open-divergence-config-button", "n_clicks"),
        Input("close-divergence-config-button", "n_clicks"),
...
def toggle_divergence_modal(n_open, n_close, n_save, is_open):
    ...
    if button_id == "open-divergence-config-button":
        return not is_open
...
@app.callback(
    ...
    [
        Input("open-open-notes-config-button", "n_clicks"),
        Input("close-open-notes-config-button", "n_clicks"),
...
def toggle_open_notes_modal(n_open, n_close, n_save, is_open):
    ...
    if button_id == "open-open-notes-config-button":
        return not is_open
...
@app.callback(
    ...
    [
        Input("open-leadtime-config-button", "n_clicks"),
        Input("close-leadtime-config-button", "n_clicks"),
...
def toggle_leadtime_modal(n_open, n_close, n_save, is_open):
    ...
    if button_id == "open-leadtime-config-button":
        return not is_open
...
def update_captura_column_store(n_clicks, selected_columns):
    ...
    if n_clicks:
        return {"visible_columns": selected_columns}
...
@app.callback(
    Output("analytic-table-graph", "figure"), Input("table-columns-store", "data")
)
...
def update_divergence_analytic_table(data):
    ...
    return create_table(
        detailed_divergence_data,
        "Tabela Analítica de Divergências",
        visible_columns=visible_columns,
    )
...
def update_open_notes_analytic_table(data):
    ...
    return create_table(
        detailed_open_notes_data,
        "Tabela Analítica de Notas em Aberto",
        visible_columns=visible_columns,
    )
...
def update_leadtime_analytic_table(data):
    ...
    return create_table(
        detailed_leadtime_data,
        "Tabela Analítica de Leadtime",
        visible_columns=visible_columns,
    )
...
def sort_list_table(n_clicks, sort_state):
    ...
    if table_id == "fornecedores":
        ...
        primary_col, metric_cols = "Razão Social Fornecedor", [
            "Total de Notas",
            "Percentual de Ingresso Manual",
        ]
        ...
        primary_col, metric_cols = "Prefeitura", [
            "Total de Notas",
            "Percentual de Ingresso Manual",
        ]
        ...
        primary_col, metric_cols = "Analista", [
            "Tarefas Concluídas",
            "Tempo Médio (min)",
        ]
        ...
        primary_col, metric_cols = "Fornecedor", [
            "Notas com Divergência",
            "Valor Total (R$)",
        ]
    ...
    if sort_col in ["Percentual de Ingresso Manual"]:
        ...
        sorted_data = sorted(
            data, key=lambda x: float(x[sort_col].replace("%", "")), reverse=reverse
        )

    elif sort_col in ["Tempo Médio (horas)"]:

        sorted_data = sorted(
            data, key=lambda x: float(x[sort_col].replace("h", "")), reverse=reverse
        )

    elif sort_col in ["Tempo Médio (min)"]:

        sorted_data = sorted(
            data, key=lambda x: float(x[sort_col].replace("m", "")), reverse=reverse
        )
        ...
        sorted_data = sorted(
            data,
            key=lambda x: float(
                x[sort_col].replace("R$ ", "").replace(".", "").replace(",", ".")
            ),
            reverse=reverse,
        )
...
@app.callback(
    ...
    [
        Input("open-manual-task-modal", "n_clicks"),
        Input("close-manual-task-modal", "n_clicks"),
    ],
...
def update_monthly_automation_chart(selected_tasks):
    ...
    for month in new_data:
        ...
        month["auto_pct"] = (
            (month["Sem Intervenção Manual"] / total * 100) if total > 0 else 0
        )

    return create_bar_line_chart(
        new_data,
        ["Sem Intervenção Manual", "Com Intervenção Manual"],
        "auto_pct",
        ["#2dd4bf", "#fb923c"],
        "#a78bfa",
        "Evolução Mensal do Automatismo",
...
@app.callback(
    Output("automation-agg-chart", "figure"), Input("automatismo-agg-dropdown", "value")
)
...
@app.callback(
    Output("captura-stacked-chart", "figure"), Input("captura-agg-dropdown", "value")
)
